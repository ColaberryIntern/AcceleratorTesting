...
- Production VPS access exists. The host address, SSH user, stack path, and deploy command are documented in the internal deploy runbook, not in this repo (this repo is public). Ask the DRI for the runbook if you need to deploy. Deploys follow the pattern: pull `main`, then rebuild the affected service via the production compose file.
...
https://github.com/ColaberryIntern/AcceleratorTesting
